...
from untitled_9 import Ui_Form
...
class MapWindow(QWidget, Ui_Form):
    def __init__(self):
        ...
        self.is_pt = False
        self.toponym_postal_code, self.toponym_address, self.toponym_coodrinates = str(), str(), str()
        ...
        self.pushButton_3.clicked.connect(self.on_or_off_postal_code)
    ...
    def FindObject(self):
        ...
        if response:
            ...
            self.is_pt = True
            self.lat2 = float(self.toponym_coodrinates.split()[1])
            self.lat = self.lat2
            self.lon2 = float(self.toponym_coodrinates.split()[0])
            self.lon = self.lon2
            self.z = 12
            self.map_request = f"http://static-maps.yandex.ru/1.x/?ll={self.lon},{self.lat}&z={self.z}&l={self.v}" \
                          f"&pt={self.lon2},{self.lat2}"
            if self.is_postal_code:
                self.label_2.setText(self.toponym_postal_code + ', ' + self.toponym_address + ', '
                                     + self.toponym_coodrinates)
            else:
                self.label_2.setText(self.toponym_address + ', ' + self.toponym_coodrinates)
        self.get_image()
        self.update()
    ...
    def on_or_off_postal_code(self):
        self.is_postal_code = False if self.is_postal_code else True
        if self.is_postal_code:
            self.label_2.setText(self.toponym_postal_code + ', ' + self.toponym_address + ', '
                                 + self.toponym_coodrinates)
        else:
            self.label_2.setText(self.toponym_address + ', ' + self.toponym_coodrinates)
        if self.is_postal_code:
            self.pushButton_3.setText('OFF почтовый индекс')
        else:
            self.pushButton_3.setText('ON почтовый индекс')
        self.get_image()
        self.update()
